...
namespace isam
{
        ...
        const VectorXd& h_gn, double delta, double& gain_ratio_denominator)
    {
        ...
        {
            gain_ratio_denominator = delta * (2 * alpha * h_sd_norm - delta)	/ (2 * alpha);
    ...
    void Optimizer::update_estimate(const Properties& prop)
    {
        ...
        else
        {
            ...
            if (alpha_denominator > 0)
            {
                ...
                VectorXd h_dl = compute_dog_leg(alpha, -gradient, -h_gn, Delta, rho_denominator);
        ...
        int max_iterations, double epsilon1, double epsilon2, double epsilon3)
    {
        ...
        while ((!found) && (max_iterations == 0 || num_iter < max_iterations))
        {
            num_iter++;
            ...
            else
            {
                ...
                double rho = (f_x.squaredNorm() - f_x_new.squaredNorm()) / (rho_denominator);
