...
#include <string.h>
#include "sensor/laser_t.h"

static int __laser_t_hash_computed;
static uint64_t __laser_t_hash;
...
uint64_t __laser_t_hash_recursive(const __lcm_hash_ptr *p)
{
    ...
    __lcm_hash_ptr cp;
    cp.parent =  p;
    cp.v = __laser_t_get_hash;
    (void) cp;

    uint64_t hash = (uint64_t)0xf1e8ba118c05af46LL
...
int64_t __laser_t_get_hash(void)
{
    if (!__laser_t_hash_computed) {
        __laser_t_hash = (int64_t)__laser_t_hash_recursive(NULL);
