...
//
CSpline::CSpline(CPoint2d& ptCenter, CPoint2d& ptStart, CPoint2d& ptEnd, CTurnDir TurnDir) :
...
void CSpline::SetCurAngle(float fPhi)
{
	...
	else if (fPhi < m_fTurnAngle - m_fBeita)
	{
		...
		CPoint2d ptTemp((float)(m_fRb * cos(fPhi)), (float)(m_fRb * sin(fPhi)));  // Local point
...
void CSpline::Solve1(float fPhi)
{
	...
	CPoint2d ptTemp((float)(fR * cos(fPhi)), (float)(fR * sin(fPhi)));   // Local point
	m_pt = m_Transform.GetWorldPoint(ptTemp);      // World point
...
void CSpline::Solve2(float fPhi)
{
	...
	// Step 2: Caculate the current point
	CPoint2d ptTemp((float)(fR * cos(fPhi)), (float)(fR * sin(fPhi)));
