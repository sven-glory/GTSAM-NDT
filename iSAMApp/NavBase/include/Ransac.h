									 ...
									 double maximalOutlierPercentage)
{
	...
	//there are less data objects than the minimum required for an exact fit, or all the data is outliers?
	...
	for (i = 0; i < numTries; i++) 
	{
      ...
      // get the indexes of the chosen objects so we can check that this sub-set hasn't been chosen already
...
template<class T, class S>
void Ransac<T,S>::computeAllChoices(ParameterEsitmator<T,S> *paramEstimator, 
												std::vector<T> &data, 
												int numForEstimate,
												short *bestVotes, 
												short *curVotes, 
...
template<class T, class S>
void Ransac<T,S>::estimate(ParameterEsitmator<T,S> *paramEstimator, 
									std::vector<T> &data, 
									int numForEstimate,
									short *bestVotes, 
									short *curVotes, 
